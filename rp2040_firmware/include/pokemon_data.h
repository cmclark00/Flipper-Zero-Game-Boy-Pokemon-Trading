...
// Flipper-specific includes removed
// #include <furi.h>
// #include <gui/icon.h>
...
struct pokemon_data {
    ...
    /* These are private to pokemon_data - Flipper specific parts commented out */
    // Storage* storage;
    // struct fxbm_sprite* bitmap; // Related to Flipper's GUI
    // uint8_t bitmap_num;          // Related to Flipper's GUI
    // FuriString* asset_path;    // Related to Flipper's asset management
};
...
// uint8_t* pokemon_icon_get(PokemonData* pdata, int num); // Commented out: uses Flipper GUI types
