...
// Adjusted include path for RP2040 project structure
#include "pokemon_data.h" // This should bring in PokemonData, EvIv (via stat_nl.h), and LEN_NAME_BUF

// These were already commented out, can be removed or left as is
//#include <src/pokemon_app.h>
//#include <src/include/pokemon_char_encode.h>

// Forward declarations for static functions defined in pokemon_data.c
// Ensure these match the actual function signatures if they are to be used across files,
// though static usually implies they are local to the .c file.
// If they are indeed local to pokemon_data.c, these declarations might not be strictly necessary here.
struct PokemonData; // Forward declare PokemonData struct if not fully defined by pokemon_data.h yet
enum EvIv;          // Forward declare EvIv enum if not fully defined by pokemon_data.h yet

static void pokemon_stat_ev_calc(struct PokemonData* pdata, enum EvIv val);
static void pokemon_stat_iv_calc(struct PokemonData* pdata, enum EvIv val);
